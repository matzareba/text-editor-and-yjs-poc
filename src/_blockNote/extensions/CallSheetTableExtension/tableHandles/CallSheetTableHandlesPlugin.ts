// https://github.com/TypeCellOS/BlockNote/blob/3e94d1a38f0a15f86d894461d7dc908ea4cc7802/packages/core/src/extensions/TableHandles/TableHandlesPlugin.ts
...
export type CallSheetTableHandlesState<
...
export class CallSheetTableHandlesView<
  I extends InlineContentSchema,
  S extends StyleSchema
> implements PluginView
{
  public state?: CallSheetTableHandlesState<I, S>;
  ...
  constructor(
    ...
    emitUpdate: (state: CallSheetTableHandlesState<I, S>) => void
  ) {
    this.emitUpdate = () => {
      if (!this.state) {
  ...
  mouseMoveHandler = (event: MouseEvent) => {
    ...
    if (block.type === callSheetTableBlockType) {
  ...
  dragOverHandler = (event: DragEvent) => {
    ...
    if (dispatchDecorationsTransaction) {
      this.editor.transact((tr) =>
        tr.setMeta(CallSheetTableHandlesPluginKey, true)
      );
    }
  };
  ...
  update() {
    ...
    if (
      !this.state.block ||
      // @ts-ignore TODO: fix this
      this.state.block.type !== callSheetTableBlockType ||
...
export const CallSheetTableHandlesPluginKey = new PluginKey(
  "CallSheetTableHandlesPlugin"
);

export class CallSheetTableHandlesProsemirrorPlugin<
  I extends InlineContentSchema,
  S extends StyleSchema
> extends BlockNoteExtension {
  public static key() {
    return "CallSheetTableHandles";
  }

  private view: CallSheetTableHandlesView<I, S> | undefined;
  ...
  constructor(
    ...
    // TODO: change this from storing this extension instance in the registry to storing it in the editor instance
    extensionRegistry.set(editor, this);

    this.addProsemirrorPlugin(
      new Plugin({
        key: CallSheetTableHandlesPluginKey,
        view: (editorView) => {
          this.view = new CallSheetTableHandlesView(
            editor,
            editorView,
            (state) => {
  ...
  public onUpdate(callback: (state: CallSheetTableHandlesState<I, S>) => void) {
  ...
  colDragStart = (event: {
    ...
    this.editor.transact((tr) =>
      tr.setMeta(CallSheetTableHandlesPluginKey, {
  ...
  dragEnd = () => {
    ...
    this.editor.transact((tr) =>
      tr.setMeta(CallSheetTableHandlesPluginKey, null)
    );
...
// TODO remove this global registry in favor of something similar to editor.tableHandles
const extensionRegistry = new WeakMap<
  any,
  CallSheetTableHandlesProsemirrorPlugin<any, any>
>();

export function getCallSheetTableHandlesExtension<
  I extends InlineContentSchema = DefaultInlineContentSchema,
  S extends StyleSchema = DefaultStyleSchema
>(
  editor: BlockNoteEditor<any, I, S>
): CallSheetTableHandlesProsemirrorPlugin<I, S> | undefined {
  return extensionRegistry.get(editor) as
    | CallSheetTableHandlesProsemirrorPlugin<I, S>
    | undefined;
}
