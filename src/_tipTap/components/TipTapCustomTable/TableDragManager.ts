...
let draggableTarget: HTMLElement | null = null;
        ...
        props: {
            ...
            state.doc.descendants((node, pos) => {
              if (node.type.name === 'customTable') {
                ...
                node.descendants((child, childPos) => {
                  ...
                  // child.descendants((cell, cellPos) => {
                  //   if(child.type.name === 'customCell') {
                  //     decorations.push(
                  //       Decoration.node(absPos + cellPos + 1, absPos + cellPos + 1 + cell.nodeSize, {
                  //         // draggable: 'true',
                  //         // class: 'draggable-cell',
                  //         'data-table-id': tableId,
                  //         'data-row-index': String(rowIndex),
                  //         // 'data-col-index': String(index),
                  //       })
                  //     )
                  //   }
                  // })
          ...
          handleDOMEvents: {
            mousedown: (view, event) => {
              draggableTarget = event.target as HTMLElement;
            ...
            dragstart(view, event) {
              ...
              if(!draggableTarget?.classList.contains('draggable-table-row')) {
                console.log('bailing out', draggableTarget);
              ...
              if (th) {
                ...
                setTimeout(() => {
                  const table = th.closest('customTable')
                  if (!table) return
            ...
            dragover(view, event) {
              ...
              if (dragType === 'col') {
                ...
                if (targetTh) {
                  const table = targetTh.closest('customTable')
